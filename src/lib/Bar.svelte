<script lang="ts">
  ...
  function getUpYear(yearRange: [undefined, undefined] | [number, number]) {
    if (!yearRange[0]) return 0;
    const timeScale = d3.scaleLinear().domain(yearRange).range([0, 100]);
  ...
  const yScale = $derived(
    ...
    d3
      ...
      .domain([0, d3.max(Object.values(genreNums))!]),
  );
      ...
      {#each Object.entries(genreNums) as [genre, cnt]}
        <g class={genre}>
          ...
          <rect
            ...
            onmouseover={() => {
                selectedGenre = genre;
                d3.select(`.${genre} .bar`).attr("opacity", 1);
            }}
            onmouseout={() => {
                selectedGenre = "";
                d3.select(`.${genre} .bar`).attr("opacity", 0.5);
            }}
          />
          ...
          <!-- tip: the text below should change with the hover on interaction -->
            {selectedGenre === genre ? `${selectedGenre}: ${cnt}`: `${cnt}`} 
          </text>
        </g>
      {/each}
    </g>
